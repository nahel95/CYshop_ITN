    ...
    if (confirmation == 'O' || confirmation == 'o') {
        ...
        while (fgets(line, sizeof(line), inputFile)) {
            if (strstr(line, "Nom du produit :") != NULL) {
                int ref;
                sscanf(line, "Nom du produit : %*s Ref : %d   Taille : %*d   Qte : %d   Prix : %*f", &ref, 
                       &tempQuantity);
        ...
        if (!foundProduct) {
            printf("Le produit achete n'existe pas dans le fichier stock.\n");
        }
        ...
        itemCount = 0;
        memset(cart, 0, sizeof(drink) * MAX_ITEMS);
        printf("Achat annule. Panier vide\n");
    }
}
