...
void finalizePurchase(drink cart[], int itemCount, customer* cust) {
    ...
    if (confirmation == 'O' || confirmation == 'o') {
        ...
        int foundProduct = 0;  // Indicateur pour vérifier si le produit a été trouvé dans le panier

        while (fgets(line, sizeof(line), inputFile)) {
            if (strstr(line, "Nom du produit :") != NULL) {
                int ref;
                sscanf(line, "Nom du produit : %*s Ref : %d   Taille : %*d   Qte : %d   Prix : %*f", &ref, &tempQuantity);

                int productUpdated = 0;  // Indicateur pour vérifier si le produit a été mis à jour

                for (int i = 0; i < itemCount; i++) {
                    if (cart[i].reference == ref) {  // Comparer la référence du produit
                        updatedQuantity = tempQuantity - cart[i].quantity;
                        if (updatedQuantity < 0) {
                            updatedQuantity = 0;  // Assurer que la quantité ne devient pas négative
                        }

                        tempQuantity = updatedQuantity;  // Mettre à jour tempQuantity

                        // Mettre à jour la quantité dans la ligne correspondante
                        fprintf(tempFile, "Nom du produit : %s   Ref : %d   Taille : %d   Qte : %d   Prix : %.2f\n",
                                cart[i].name, cart[i].reference, cart[i].size, updatedQuantity, cart[i].price);

                        foundProduct = 1;  // Marquer que le produit a été trouvé
                        productUpdated = 1;  // Marquer que le produit a été mis à jour
                    }
                }

                if (!productUpdated) {
                    // Copier les lignes non concernées directement dans le fichier temporaire
        ...
        if (!foundProduct) {
            printf("Le produit acheté n'existe pas dans le fichier stock.\n");
        }
        ...
        FILE* purchaseFile = fopen(filename, "a"); // Ouvrir le fichier en mode "append"
        ...
        fprintf(purchaseFile, "Historique des achats :\n");
        for (int i = 0; i < itemCount; i++) {
            fprintf(purchaseFile, "Nom du produit : %s   Ref : %d   Taille : %d   Qte : %d   Prix : %.2f\n",
                    cart[i].name, cart[i].reference, cart[i].size, cart[i].quantity, cart[i].price);
        }
        fclose(purchaseFile);

        printf("Achat effectué avec succès ! Panier vidé\n");
        memset(cart, 0, sizeof(drink) * MAX_ITEMS);
        itemCount = 0;
    } else {
        itemCount = 0;
        memset(cart, 0, sizeof(drink) * MAX_ITEMS);
        printf("Achat annulé. Panier vidé. Déconnexion du Mode Achat.\n");
    }
}
